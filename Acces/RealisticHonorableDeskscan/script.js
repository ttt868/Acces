...
function updateHalvingMilestones(circulatingSupply) {
  ...
  milestones.forEach(function(el) {
    ...
    // Ensure NOW tag and glow only on active
  ...
  // Update track fill (progress across all phases)
  ...
  if (progressInner) {
    ...
    if (circulatingSupply > 0 && phasePercent < 2) phasePercent = 2; // minimum visible
  ...
  // Update remaining to next halving
...
// Fetch circulating supply and update milestones
function fetchHalvingData() {
  fetch('/api/network/network-info')
...
// Instantly show cached value (no -- or flicker), then refresh in background
(function() {
  try {
...
function startGradualAccumulation() {
  ...
  async function fetchBoostData() {
    try {
      ...
      const response = await fetch(`/api/processing/accumulated/${currentUser.id}?st=${encodeURIComponent(st)}`);
      ...
      if (loginContainer && appContainer) {
        ...
        if (currentUser.id) {
          ...
          initializeUserWallet().then(function() {
            console.log('[PreloadQR] Wallet ready, QR pre-generated');
          }).catch(function(e) { console.warn('[PreloadQR] Wallet init skipped:', e); });
        }
      }
    });
  }
...
function initializeGoogleSignIn() {
    ...
    try {
      ...
      try {
        ...
        if (walletAddress && fallbackWallet && fallbackWallet.publicAddress) {
          ...
          // Generate QR code for fallback wallet
          if (fallbackWallet.publicAddress) {
            generateAndSaveQRCode(fallbackWallet.publicAddress);
          }
  ...
  // Fallback QR: draw a deterministic pattern on canvas when QRCode library fails
  function _drawQRFallbackCanvas(qrDiv, address) {
    qrDiv.innerHTML = '';
    const canvas = document.createElement('canvas');
    canvas.width = 150;
    canvas.height = 150;
    ...
    // Generate a deterministic hash from address
    let hash = 0;
    ...
    // Draw data modules (unique pattern per address)
    const moduleSize = 5;
    const modules = 25;
    ctx.fillStyle = '#000000';
    for (let row = 0; row < modules; row++) {
      for (let col = 0; col < modules; col++) {
        ...
        if ((row < 7 && col < 7) || (row < 7 && col >= 18) || (row >= 18 && col < 7)) continue;
        const seed = (row * 31 + col * 37 + hash) & 0x7fffffff;
        if (seed % 3 !== 0) {
          ctx.fillRect(col * moduleSize + 12, row * moduleSize + 12, moduleSize, moduleSize);
        }
      }
    }

    // Draw finder patterns (standard QR corners)
    ...
    drawFinder(12, 12);   // top-left
    drawFinder(103, 12);  // top-right
    drawFinder(12, 103);  // bottom-left

    console.log('QR fallback canvas drawn for:', address.substring(0, 10) + '...');
  }

  // Save QR code HTML to session/server after rendering (delayed to ensure image exists)
  function _saveQRCodeHTML(qrContainer, cleanAddress) {
    // Delay save to ensure async QR image is fully rendered in DOM
    setTimeout(function() {
      try {
        // Only save if there's an actual QR image/canvas
        var qrDisplay = qrContainer.querySelector('#qrcode-display');
        var hasReal = qrDisplay && (qrDisplay.querySelector('img') || qrDisplay.querySelector('canvas'));
        if (!hasReal) {
          console.warn('_saveQRCodeHTML: No real QR image found yet, skipping save');
          return;
        }
        var qrCodeData = qrContainer.innerHTML;
        ...
        console.log('_saveQRCodeHTML: Saved QR with real image');
      } catch (e) {
        console.warn('Error saving QR HTML:', e);
      }
    }, 800);
  }
  ...
  // Helper function to generate QR code — static HTML exists, just update img src
  function generateAndSaveQRCode(address) {
    try {
      if (!address) return false;
      var cleanAddress = address.trim();
      var img = document.getElementById('qr-img');
      if (!img) { console.warn('QR: #qr-img not found'); return false; }

      // Already rendered for this address — skip (case-insensitive for ETH addresses)
      var cachedAddr = (img.dataset.addr || '').toLowerCase();
      var newAddr = cleanAddress.toLowerCase();

      // Always ensure address text is shown
      var at = document.getElementById('qr-address-text');
      if (at && cleanAddress.length > 14) {
        at.textContent = cleanAddress.substring(0,8) + '....' + cleanAddress.substring(cleanAddress.length-6);
      }
      ...
      console.log('QR: Generating for', cleanAddress);

      // Try node-qrcode first
      if (window._nodeQR && typeof window._nodeQR.toDataURL === 'function') {
        window._nodeQR.toDataURL(cleanAddress, {
          width: 150, margin: 1,
          color: { dark: '#000000', light: '#ffffff' }
        }, function(err, dataUrl) {
          if (err || !dataUrl) { console.error('QR toDataURL failed:', err); return; }
          // Double-check inside callback: if image already has valid QR, don't touch it
          var currentAddr = (img.dataset.addr || '').toLowerCase();
          if (currentAddr === newAddr && img.src && img.src.indexOf('data:image/png') === 0) {
            console.log('QR: Already set before callback, skipping update');
            return;
          }
          img.src = dataUrl;
          img.dataset.addr = cleanAddress;
          var at = document.getElementById('qr-address-text');
          if (at) at.textContent = cleanAddress.substring(0,8) + '....' + cleanAddress.substring(cleanAddress.length-6);
          try {
            localStorage.setItem('last_qr_data', dataUrl);
            localStorage.setItem('last_qr_addr', cleanAddress);
            if (currentUser && currentUser.id) {
          ...
          if (currentUser && currentUser.id) {
            var c2 = document.querySelector('.qrcode-container');
            saveQRCodeToServer(currentUser.id, c2 ? c2.innerHTML : '', cleanAddress);
          }
      ...
      // Fallback: try QRCode (qrcodejs)
      if (window.QRCode) {
        var canvas = document.createElement('canvas');
        new window.QRCode(canvas, { text: cleanAddress, width: 150, height: 150 });
        setTimeout(function() {
          try {
            var cvs = canvas.querySelector('canvas');
            if (cvs) { img.src = cvs.toDataURL(); img.dataset.addr = cleanAddress; }
          } catch(e) {}
        }, 100);
        return true;
      }

      console.warn('QR: No QR library available');
      return false;
    } catch (error) {
...
`;
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Network page nav click — showPage('network') handles everything
    // This listener just ensures balance refreshes on repeated clicks
    document.querySelectorAll('.nav-link, .mobile-nav-item').forEach(link => {
      link.addEventListener('click', function() {
        var pageName = this.getAttribute('data-page');
        if (pageName === 'network' && currentUser && currentUser.id) {
          // showPage will call initializeUserWallet + generateAndSaveQRCode
    ...
    if (document.getElementById('network-page') && 
      ...
      setTimeout(function() {
        if (currentUser && currentUser.id) {
          // QR generated inside initializeUserWallet automatically
          initializeUserWallet().then(function() {
            // wallet + QR initialized
          }).catch(function(e) { console.error('[DOMContentLoaded] Wallet init failed:', e); });
        }
      }, 500);
    }

    // Handle page visibility changes — only refresh balance, not QR code
    document.addEventListener('visibilitychange', function() {
      if (document.visibilityState === 'visible') {
        var networkPage = document.getElementById('network-page');
        if (networkPage && networkPage.style.display !== 'none' && currentUser && currentUser.id) {
          // Just refresh wallet data, QR is static
  ...
  function continueWithLogin(user, referralCode) {
    ...
    loadUserData(user.email, true).then(() => {
      ...
      if (currentUser.id) {
        ...
        // Pre-generate QR code in background so it's ready before visiting Network
        initializeUserWallet().then(function() {
          console.log('[PreloadQR] Wallet ready after login, QR pre-generated');
        }).catch(function(e) { console.warn('[PreloadQR] Wallet init skipped:', e); });
