          ...
          try {
            ...
            if (resp.status === 401 && data.requireRelogin) {
              ...
              try {
                ...
                const _origin = (typeof window.getApiOrigin === 'function') ? window.getApiOrigin() : window.location.origin;
                const freshResp = await fetch(_origin + '/api/user/' + encodeURIComponent(currentUser.email));
      ...
      if (loginContainer && appContainer) {
        ...
        if (currentUser.id) {
          ...
          initializeUserWallet().then(function() {
            console.log('[Session Restore] QR pre-generated');
          }).catch(function(e) { console.error('[Session Restore] QR pre-gen failed:', e); });
        }
      }
    });
  }
  ...
  async function loadUserData(email, isFreshLogin) {
    ...
    try {
      ...
      if (userData) {
        ...
        // On session restore (page reload, back from .html, app reopen), just sync
        // the existing token from server — do NOT generate a new one.
        if (currentUser.id) {
          if (isFreshLogin) {
            try {
...
function initializeGoogleSignIn() {
  ...
  async function initializeUserWallet() {
    ...
    try {
      ...
      try {
        const _st = encodeURIComponent(currentUser.sessionToken || currentUser.session_token || '');
        const response = await fetch(`${(typeof getApiOrigin !== "undefined" ? getApiOrigin() : window.location.origin)}/api/user/wallet-key/${currentUser.id}?session_token=${_st}`);
        if (response.ok) {
    ...
    document.addEventListener('visibilitychange', function() {
      if (document.visibilityState === 'visible') {
        ...
        if (networkPage && networkPage.style.display !== 'none' && currentUser && currentUser.id) {
          ...
          initializeUserWallet().catch(function(e) {});
        }
      }
    });
  });
